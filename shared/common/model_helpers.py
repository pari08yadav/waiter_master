...
qr_base_url = f"http://{settings.BASE_URL}/table/"
...
def generate_username():
    adjectives = ["Cool", "Fast", "Silly", "Smart", "Brave", "Clever", "Sneaky"]
...
def generate_chain_name():
    adjectives = ["Tasty", "Golden", "Spicy", "Savory", "Fresh", "Urban", "Classic"]
...
def attach_qr(text: str):
    qrcode_obj = qrcode.QRCode(error_correction=qrcode.constants.ERROR_CORRECT_H)
    qrcode_obj.add_data(f"{qr_base_url}{text}/")
    qrcode_obj.make(fit=True)
    qr_img = qrcode_obj.make_image(
        image_factory=StyledPilImage, module_drawer=CircleModuleDrawer()
    )
    bytes_buffer = io.BytesIO()
    qr_img.save(bytes_buffer, "PNG")
    return bytes_buffer
